...
load_env_config() {
    if [[ -s "$1" ]]; then
        ...
        [[ -z "$SRC_PROFILE" ]] && SRC_PROFILE="$(grep '^source_profile' "$1"|cut -d= -f2)"
        if [[ -z "$ROLE_ARN" ]]; then
...
[[ -n "$SRC_PROFILE" ]] || SRC_PROFILE="$(sed -ne '/\['"$PROFILE_SECTION"'\]/,/^\[/p' "$AWS_CONFIG_FILE"|grep '^source_profile'|cut -d= -f2)"
