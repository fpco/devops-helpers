...
cleanup() {
    ...
    # When running in the bash PROMPT_COMMAND, allow user to abort or retry if
    # acquiring a temporary sesssion fails.
    if [[ $EVAL_IN_PROMPT == 1 && $COMPLETED_SUCCESSFULLY == 0 ]]; then
        echo "[$(basename "$0")] Did not complete successfully." >&2
        local arf
        arf=
        while [[ -z "$arf" ]]; do
            read -p "[$(basename "$0")] Abort, Retry, Fail?" -r arf </dev/tty
            case "$arf" in
                [aA]*) echo "PROMPT_COMMAND=\"\$AWS_ENV_ORIG_PROMPT_COMMAND\""
                       break
                       ;;
                [rR]*) exec "$0" "${ORIG_ARGS[@]}"
                       ;;
                [fF]*) break
                       ;;
                *)
                    echo "[$(basename "$0")] Invalid response." >&2
                    echo "[$(basename "$0")]   a = disable auto prompt mode" >&2
                    echo "[$(basename "$0")]   r = try again right away" >&2
                    echo "[$(basename "$0")]   f = fail this time, but try again next time" >&2
                    ;;
            esac
        done
    fi
...
load_env_config() {
    if [[ -s "$1" ]]; then
        ...
        [[ -n "$PROMPT_MODE" ]] || \
            PROMPT_MODE="$(configfield "$1" prompt_mode)"
    fi
}

# 'setup_shell_prompt' modifies the bash prompt environment variables for
# inclusion of the profile name and to support '--auto' mode.
# TODO: support other shells besides 'bash'
setup_shell_prompt() {
    if [[ $EVAL_IN_PROMPT == 0 ]]; then
        ...
        echo "if [ -z \${AWS_ENV_ORIG_PROMPT_COMMAND+x} ]; then export AWS_ENV_ORIG_PROMPT_COMMAND=\"\$PROMPT_COMMAND\"; else PROMPT_COMMAND=\"\$AWS_ENV_ORIG_PROMPT_COMMAND\"; fi;"

        # For '--auto' mode, initialize state and set PROMPT_COMMAND
        if [[ "$PROMPT_MODE" == "auto" ]]; then
            echo "export AWS_ENV_PROMPT_PWD=;"
            echo "export AWS_ENV_PROMPT_CONFIG_FILE=;"
            echo "case \"\$PROMPT_COMMAND\" in *$(basename "$0")*eval-in-prompt*) ;; *) PROMPT_COMMAND='eval \$(\"$0\" --eval-in-prompt $(printf '%q ' "${ORIG_ARGS[@]}"));'\"\$PROMPT_COMMAND\" ;; esac;"
        fi

        # Add the current profile to PS1
        [[ "$PROMPT_MODE" == "no" || -z "$PROMPT_FORMAT" ]] || \
...
#
# --eval-in-prompt is a special internal option that changes behaviour of the
# script for when its output is auto-evaluated before the shell prompt.
#

if [[ "${1:-}" == "--eval-in-prompt" ]]; then
    # Exit immediately nothing if we haven't changed directory.
    if [[ "$PWD" == "$AWS_ENV_PROMPT_PWD" && "$CURDATE" -le "${AWS_ENV_EXPIRE:-$CURDATE}" ]]; then
        COMPLETED_SUCCESSFULLY=1
        exit 0
    fi
    echo "export AWS_ENV_PROMPT_PWD='$PWD';"
    EVAL_IN_PROMPT=1
    shift
fi
...
if [[ $EVAL_IN_PROMPT == 1 ]]; then
    # Exit immediately if we haven't changed config files.
    if [[ "$FIRST_ENV_CONFIG_FILE" == "$AWS_ENV_PROMPT_CONFIG_FILE" && "$CURDATE" -le "${AWS_ENV_EXPIRE:-$CURDATE}" ]]; then
        COMPLETED_SUCCESSFULLY=1
        exit 0
    fi
    [[ "$FIRST_ENV_CONFIG_FILE" == "$AWS_ENV_PROMPT_CONFIG_FILE" ]] || echo "[$(basename "$0")] Switching to config in ${FIRST_ENV_CONFIG_FILE:-default}" >&2
    echo "export AWS_ENV_PROMPT_CONFIG_FILE='$FIRST_ENV_CONFIG_FILE';"
fi
...
[[ -n "$PROMPT_MODE" ]] || PROMPT_MODE=simple
...
#
# Setup the shell or '--auto' mode (and exit if enabled)
#

if [[ "$PROMPT_MODE" == "auto" && $EVAL_IN_PROMPT == 0 && $# -eq 0 ]]; then
    setup_shell_prompt
    COMPLETED_SUCCESSFULLY=1
    exit 0
fi
...
if [[ "$(grep '^\['"$SRC_PROFILE_SECTION"'\]' "$AWS_CONFIG_FILE")" == "" && "$(grep '^\['"$SRC_PROFILE"'\]' "$HOME/.aws/credentials")" == "" ]]; then
    ...
    if [[ $EVAL_IN_PROMPT == 0 ]]; then
        ...
        # If we're being eval'd in the shell prompt, moving to a config with no
        # profile just unsets all the credentials variables.
        echo "unset AWS_ACCESS_KEY_ID;"
        echo "unset AWS_SECRET_ACCESS_KEY;"
        echo "unset AWS_SESSION_TOKEN;"
        echo "unset AWS_SECURITY_TOKEN;"
        echo "unset AWS_ENV_EXPIRE;"
        echo "unset AWS_ENV_CURRENT_PROFILE;"
        COMPLETED_SUCCESSFULLY=1
        exit 0
    fi
fi
...
if [[ ! -s "$MFA_CRED_FILE" || "$CURDATE" -ge "$AWS_ENV_EXPIRE" ]]; then
    ...
    if [[ -z "$ROLE_ARN" && -z "$MFA_SERIAL" && $EVAL_IN_PROMPT == 0 ]]; then
